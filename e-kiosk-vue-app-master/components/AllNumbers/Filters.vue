...
export default {
  data() {
    return {
      ...
      numbers: [{ value: "Filteri sil", clear: true }],
      yearKey: 0,
      numberKey: 0,
      monthKey: 0,
      dayKey: 0,
  ...
  methods: {
    selectNumber(value) {
      if (value.clear) {
        this.filter.numberModel = null;
        this.numberKey++;
      } else {
        this.filter.numberModel = value;
      }
  ...
  computed: {
    getNumberList() {
      return this.$store.getters["all-numbers/getNumberList"];
    },
  },
};
</script>
